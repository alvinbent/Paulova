        ...
        <div className="border-b border-[#b99862]/16 p-5 md:p-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-4">
              ...
              <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#5f4f42] text-[#fffdf8]">
                <IconoDoctor name={icon} className="h-7 w-7" />
              </span>
              <div>
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href={`/doctor/pacientes/${patientId}`}
                className="paunova-button-secondary inline-flex items-center justify-center gap-2 rounded-full px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.16em]"
              >
                <IconoDoctor name="arrow_back" className="h-4 w-4" />
                Regresar
              </Link>
          ...
          <nav className="border-b border-[#b99862]/16 p-4 xl:border-b-0 xl:border-r">
            <div className="grid grid-cols-2 gap-2 xl:grid-cols-1">
              {nav.map(([label, href, navIcon]) => (
                ...
                >
                  <IconoDoctor name={navIcon} className="h-4 w-4" />
                  <span>{label}</span>
                </Link>
              ))}
            </div>
          ...
          <section className="min-w-0 border-b border-[#b99862]/16 p-5 xl:border-b-0 xl:border-r">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <button className="paunova-button-secondary inline-flex items-center justify-center gap-2 rounded-full px-4 py-2.5 text-[10px] font-semibold uppercase tracking-[0.14em]">
                <IconoDoctor name="filter_list" className="h-4 w-4" />
                Filtrar
              </button>
            </div>
          ...
          <aside className="p-5">
            ...
            <div className="mt-5 space-y-3">
              {items.map((item) => (
                ...
                >
                  <IconoDoctor name="task_alt" className="h-4 w-4 mt-0.5 text-[#b99862]" />
                  <span>{item}</span>
                </div>
              ))}
            </div>
